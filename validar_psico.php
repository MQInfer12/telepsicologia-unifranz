    ...
    if($_POST['contrasena'] == $_POST['ccontrasena']) {
        if(strlen($_POST['correo']) >= 1 && strlen($_POST['usuario']) >= 1) {
            if(strlen($_POST['contrasena']) >= 8) {
                ...
                $consulta = "INSERT INTO tbl_login (correo, usuario, contrasena, rol, validar) VALUES ('$correo', '$usuario' , '$hash', 'psicologo', '0')";
                $resultado = mysqli_query($conexion, $consulta);
                if($resultado) {
                    ?>
                    <script>
                ...
                <script>
                    Swal.fire({
                    icon: 'error',
                    text: 'La contrasena debe tener mínimo 8 carácteres.',
                    })
                </script>
                <?php
            }
